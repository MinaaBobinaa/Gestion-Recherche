...
RecetteLongue trouverRecetteLaPlusLongue(FILE* fichier) {
    ...
    char ligne[MAX_LONGUEUR_LIGNE];
    rewind(fichier); // S'assurer de commencer à lire depuis le début du fichier

    while (fgets(ligne, sizeof(ligne), fichier) != NULL) {
        // Trouver la position de la première crochète pour isoler le nom de la recette
        char* posCrochete = strchr(ligne, '[');
        int longueurNom = (posCrochete != NULL) ? (int)(posCrochete - ligne) : strlen(ligne);

        // Exclure le caractère de nouvelle ligne, si présent
